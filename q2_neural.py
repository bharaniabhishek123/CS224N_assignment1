...
def forward_backward_prop(X, labels, params, dimensions):
    ...
    cost = - np.sum(labels *np.log(ybar)) # \CE = -sum(y*log ybar)
    ...
    derv_ce = ybar - labels # (ybar-y)

    gradW2 = np.dot(h.T, derv_ce)
    gradb2 = np.sum(derv_ce, axis=0, keepdims=True)
    dh = np.dot(derv_ce, W2.T)
